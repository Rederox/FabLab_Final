...
int main() {
	...
	struct sockaddr_in servaddr, cliaddr;	//on créer deux structures 
	...
	while(1){
		...
		if(temp.compare(TestWord) == 0){			//réception mot pour valider l'initialisation de la communication 
			...
			time(&t1);	//récupération de l'heure avant le commencement de la boucle
			//tant que le badge n'a pas été détecté et que le délai de 10 secondes n'est pas dépassé
			while(detection != 1 && seconds<10){			
			//similaire au code de la conception n°1 ( Gestion des accès des entrées du FABLAB )
			//-------------------------------------------------------------------------------------//
			...
			//-------------------------------------------------------------------------------------//
				...
				time(&t2);	//récupération de l'heure pendant l'execution de la boucle
				seconds = difftime(t2,t1);	// calcul différence de temps entre les deux variables en secondes 
				sleep(1);
